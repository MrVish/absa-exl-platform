...
resource "aws_cloudwatch_log_metric_filter" "iam_policy_change" {
  name           = "exl-stg-iam-policy-change"
  log_group_name = aws_cloudwatch_log_group.cloudtrail.name
...
resource "aws_cloudwatch_log_metric_filter" "sts_assume_role_fail" {
  name           = "exl-stg-sts-assume-role-fail"
  log_group_name = aws_cloudwatch_log_group.cloudtrail.name
...
resource "aws_cloudwatch_log_metric_filter" "cloudtrail_change" {
  name           = "exl-stg-cloudtrail-change"
  log_group_name = aws_cloudwatch_log_group.cloudtrail.name
...
resource "aws_cloudwatch_metric_alarm" "root_usage" {
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "Root account usage detected in exl-stg"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.security_alerts.arn]
}

resource "aws_cloudwatch_metric_alarm" "iam_policy_change" {
  alarm_name          = "exl-stg-iam-policy-change"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "IAM policy change detected in exl-stg"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.security_alerts.arn]
}

resource "aws_cloudwatch_metric_alarm" "kms_cmk_change" {
  alarm_name          = "exl-stg-kms-cmk-change"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "KMS key disabled or scheduled for deletion in exl-stg"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.security_alerts.arn]
}

resource "aws_cloudwatch_metric_alarm" "s3_policy_change" {
  alarm_name          = "exl-stg-s3-policy-change"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "S3 bucket policy change detected in exl-stg"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.security_alerts.arn]
}

resource "aws_cloudwatch_metric_alarm" "sts_assume_role_fail" {
  alarm_name          = "exl-stg-sts-assume-role-fail-burst"
  comparison_operator = "GreaterThanOrEqualToThreshold"
...
resource "aws_cloudwatch_metric_alarm" "cloudtrail_change" {
  alarm_name          = "exl-stg-cloudtrail-change"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Sum"
  threshold           = 1
  alarm_description   = "CloudTrail config change detected in exl-stg (StopLogging / DeleteTrail / UpdateTrail)"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.security_alerts.arn]
}
...
locals {
  permissions_boundary_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/stg-env-scoped-boundary"
}
