...
Per spec section 6.3. The chain is:
  ...
  3.7 generate-pipeline register
...
def run_producer_chain(
    ...
    # publish-key writes to s3://<bucket>/manifest-signing/<key_id>/<version>.pem
    # — see manifest_signer.publisher.publish_public_key. Verifier needs this
    # exact path because the bucket policy only grants public read on
    # manifest-signing/*.
    _key_id = endpoints.kms_key_arn.rsplit("/", 1)[-1]
    public_key_uri = f"s3://{endpoints.public_key_bucket}/manifest-signing/{_key_id}/v1.pem"

    # 3.7 generate-pipeline register
    # The register subcommand of generate-pipeline POSTs the registration
    # body (generated by `generate-pipeline generate`) to the registry API
    # via SigV4. Output is a Python dict str like:
    #   {'status': 'created', 'body': {'pk': '...', 'sk': '...', ...}}
    # We don't have a record_id field in the response (registry uses
    # composite key pk/sk); we extract the status to verify success and
    # treat the full body str as the "record_id" for transcript purposes.
    out_3_7 = _run_cli(
        [
            "uv",
            "run",
            ...
            "register",
            "--pipeline",
            "credit-risk-pd@1.0.0",
            "--endpoint",
            endpoints.registry_url,
        ],
        step_name="3.7 generate-pipeline register",
        transcript=transcript,
    )
    out_text = out_3_7.decode("utf-8", errors="replace").strip()
    # Sanity-check the response indicates success (created or already_exists).
    if "'status': 'created'" not in out_text and "'status': 'already_exists'" not in out_text:
        raise DemoStepFailed(
            step="3.7 register-pipeline check",
            account="exl-prod-sim",
            exit_code=2,
            stdout=out_3_7,
            stderr=b"register did not return status=created or already_exists",
            hint=(
                "Check the registry-api logs (infra/localstack/.uvicorn.log) "
                "for the actual error. Common: TABLE_NAME env not propagated, "
                "or SigV4 signing failed because uvicorn doesn't have "
                "absa-exl auth context."
            ),
        )
    record_id = out_text  # full response str; used by T11 verifier for cross-check
